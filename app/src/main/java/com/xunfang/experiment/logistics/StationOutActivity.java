 ...
 * 
 */
public class StationOutActivity extends Activity implements OnClickListener{
	//定义控件
	private Sub_sys_main_common stationSubSysMainItem;
	private ImageView revert;//返回"按钮"
	private TextView title;//标题
	//更新界面的Handler
	private static RefreshHandler StationOut_RefreshHandler = null;
	//界面中的TextView数组
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //设置界面内容
        setContentView(R.layout.station_out);
        //自定义窗口样式
		...
		title.setText(getText(R.string.inoroutMgr)+"——"+getText(R.string.station_out));
		revert.setOnClickListener(this);
        //获得子系统公用主界面控件的实例
		stationSubSysMainItem = (Sub_sys_main_common)findViewById(R.id.sub_sys_stationout);
        //测试用、设置界面显示内容
	...
	protected void onResume() {
		...
		if("".equals(MyConfig.getNOW_LOGINNAME())){
    		//Intent intent = new Intent(StationOutActivity.this,LoginActivity.class);
			Intent intent = new Intent(StationOutActivity.this,Main.class);
			Main.setChecked(R.id.main_tab_login);
    		startActivity(intent);
    		finish();
    	}else{
			//设置系统当前所处界面
			MyConfig.setCurrent_interface(MyConfig.STATION_OUT);
			StationOut_RefreshHandler = new RefreshHandler(textView_arr,
			...
			if (ReaderConfig.getUART_STATE() > 0) {// 判断串口是否打开
				...
				Thread scThread = new Thread(new SearchCardThread(StationOutActivity.this,
	...
	public static void ParseMessage(Bundle message) {
		...
		msg.setData(message);
		StationOut_RefreshHandler.sendMessage(msg);
	}
	@Override
		...
		if(view instanceof ImageView){
			...
			case R.id.common_title_revert:
				/*if(MyConfig.getNOW_USERTYPE() == 0){//管理员
					startActivity(new Intent(StationOutActivity.this,GoodsInorOutActivity.class));
